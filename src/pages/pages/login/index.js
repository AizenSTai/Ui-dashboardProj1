...
const LoginPage = () => {
  ...
  const loginOnClick = async event => {
    ...
    if (user.length > 3 && pass.length > 3) {
      const Send = await fetch('https://nodex.microsis.net/user/login', {
        method: 'POST',
        body: data,
        headers: {
      ...
      console.log('لطفا اطلاعات وارده را تصحیح نمایید')
      setIsLoading(false)
    }
  }
      ...
      <Box>
        <Card sx={{ zIndex: 1 }}>
          <CardContent sx={{ padding: theme => `${theme.spacing(12, 9, 7)} !important` }}>
            ...
            <SnackbarPopUp ref={childRef} severity={'success'} message={'it has succeeded'} />
