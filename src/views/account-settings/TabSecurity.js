...
const TabSecurity = () => {
  ...
  const onPassChangehandler = async prop => {
    ...
    if (tkn !== null && tkn !== undefined) {
      if (newpassA.length >= 2 && newpassA === newpassB) {
        const Send1 = await fetch('http://nodex.microsis.net/user/Profile', {
          method: 'POST',
          headers: {
        ...
        if (Send1 != undefined) {
          const Response = await Send1.json()
          const data1 = { ...Response.data }
          console.log(data1[0].username)
          const Send2 = await fetch('http://nodex.microsis.net/user/login', {
            method: 'POST',
            body: JSON.stringify({ username: data1[0].username, password: currentpass }),
            headers: {
              'Access-Control-Allow-Origin': '*',
              'Content-type': 'application/json',
              'Access-Control-Allow-Headers': 'Content-Type',
          ...
          if (Send2.status == 200) {
            const Send = await fetch('http://nodex.microsis.net/user/ChangePassword', {
              method: 'POST',
              body: data,
              headers: {
                'Access-Control-Allow-Origin': '*',
  ...
  return (
    <form>
      ...
      <CardContent>
        ...
        <Box sx={{ mt: 11 }}>
          <Button variant='contained' onClick={onPassChangehandler} sx={{ marginRight: 3.5 }}>
            Save Changes
          </Button>
          <Button
            type='reset'
